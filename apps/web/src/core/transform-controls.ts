...
class TransformControls extends THREE.Controls<ControlsEvent> {
  ...
  attach(objects: THREE.Object3D[]): TransformControls {
    ...
    console.log("[TransformControls] Group center position:", center);
    ...
    // 그룹에 객체 추가
    objects.forEach((object) => {
      // attach 전 오브젝트의 월드 좌표
      const worldPosBefore = new THREE.Vector3();
      object.getWorldPosition(worldPosBefore);

      _attach(this._group!, object);

      // attach 후 오브젝트의 로컬 좌표 (그룹에 대한 상대 좌표)
      console.log(
        `[TransformControls] Object "${object.name || object.uuid}" attached to group:`,
        {
          worldPositionBefore: worldPosBefore,
          groupPosition: this._group.position,
          objectLocalPosition: object.position,
          objectLocalRotation: object.rotation,
          objectLocalScale: object.scale,
        }
      );
...
function getPointer(
  event: THREE.Event
): Object.assign<THREE.Vector2, { button: number }> {
